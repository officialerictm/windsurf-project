...
print_info() { 
    # Simple info message without color codes
    echo "INFO: $1" >&2
}
...
# Safe chown that won't fail on exFAT filesystems
safe_chown() {
    # Only attempt chown if the filesystem supports it (not exFAT)
    local path="$1"
    local fs_type=""
    
    # Get filesystem type of the path
    if [ -e "$path" ]; then
        fs_type=$(df -T "$path" 2>/dev/null | awk 'NR==2 {print $2}' || echo "")
    fi
    
    # Skip chown for exFAT or if we can't determine filesystem
    if [ "$fs_type" = "exfat" ] || [ -z "$fs_type" ]; then
        return 0
    fi
    
    # For supported filesystems, run chown
    sudo chown "$@"
}
spinner() {
...
ask_format_usb() {
    ...
    ask_yes_no_quit "Do you want to format the USB drive $usb_device?\n   (RECOMMENDED for new setups. ALL DATA ON $usb_device WILL BE LOST!)" FORMAT_USB_CHOICE
...
check_disk_space() {
    ...
    print_warning "Placeholder check_disk_space() called for USB base path: ${C_BOLD}$USB_BASE_PATH${C_RESET}." >&2
    print_info "This function should check available disk space against estimated required space." >&2
...
ask_target_os_binaries() {
    # Simple header without color codes
    echo ""
    echo "=== SELECT OS BINARIES TO INSTALL ==="
    echo "Which Ollama runtimes do you want to include on the USB?"
    
    # Define options
    echo "  1) All (Linux, macOS, Windows) (Default)"
    ...
    while true; do
        # Clear input buffer
        while read -t 0 -r; do :; done
        
        # Simple prompt
        echo -n "Enter your choice (1-5, or q) [Default is 1]: "
        
        # Read input with a timeout to handle no input
        ...
        # Trim whitespace and set default if empty
        ...
        case "$choice_lc" in
            ...
            2|c|current)
                case "$(uname -s)" in
                    ...
                    *)
                        echo ""
                        echo "WARNING: Host OS ($(uname -s)) not directly mappable to a single binary target."
                        echo "Please choose another option or select 'All'."
                        echo ""
                        continue 2
                        ;;
                esac
                SELECTED_OS_TARGETS="$current_os_tag"
                echo "Will install binaries for current host OS: $current_os_tag"
                break
                ;;
            3|l|linux) 
            ...
            *)
                echo ""
                echo "Invalid input. Please enter a number from 1 to 5, or q."
                echo "You can also type 'all', 'current', 'linux', 'mac', or 'windows'."
                echo ""
                ;;
        esac
    done
    
    # Calculate and display size estimate without color codes
    local estimated_binaries_size_bytes=$(calculate_estimated_binary_size_bytes)
    ESTIMATED_BINARIES_SIZE_GB=$(awk "BEGIN {printf \"%.2f\", $estimated_binaries_size_bytes / (1024*1024*1024)}")
    echo ""
    echo "SUCCESS: Selected OS targets for binaries: $SELECTED_OS_TARGETS (Est. Size: $ESTIMATED_BINARIES_SIZE_GB GB)"
    echo ""
}
...
generate_webui_html() {
    ...
    safe_chown "$(id -u):$(id -g)" "$usb_base_dir/webui"
...
generate_security_readme() {
    ...
    safe_chown "$(id -u):$(id -g)" "$usb_base_dir/Installation_Info"
...
generate_checksum_file() {
    ...
    safe_chown "$(id -u):$(id -g)" "$checksum_file"
...
while true; do
        ...
        create_new)
            ...
            if [[ "$FORMAT_USB_CHOICE" == "yes" ]]; then
                ...
                if [[ "$PROCEED_FORMAT_CHOICE" == "yes" ]]; then
                    print_info "⚙️ Formatting $USB_DEVICE_PATH (Placeholder - actual formatting logic needed here)..."
                    print_warning "Placeholder: Actual formatting of $USB_DEVICE_PATH is SKIPPED."
                    print_info "Assuming formatting was successful and an exFAT partition was created."
                    if [[ "$(uname)" == "Linux" ]]; then
                        ...
                        print_info "Placeholder: Linux partition path set to $USB_PARTITION_PATH"
                    elif [[ "$(uname)" == "Darwin" ]]; then
                        print_info "Placeholder: macOS formatting would target $USB_DEVICE_PATH with label $USB_LABEL_DEFAULT"
                    fi
                    USB_LABEL="$USB_LABEL_DEFAULT"
                    print_success "Formatting process (placeholder) complete."
                    USB_BASE_PATH=""
                    if ! ensure_usb_mounted_and_writable "$USB_PARTITION_PATH"; then print_fatal "Failed to mount USB after formatting (placeholder)."; fi
                else
                    ...
                    if ! ensure_usb_mounted_and_writable "$USB_PARTITION_PATH"; then print_fatal "Failed to mount unformatted USB (placeholder)."; fi
                fi
            ...
            safe_chown -R "$(id -u):$(id -g)" "$USB_BASE_PATH/Data" "$USB_BASE_PATH/webui" "$USB_BASE_PATH/.ollama" "$USB_BASE_PATH/Installation_Info"
            ...
            safe_chown -R "$(id -u):$(id -g)" "$USB_BASE_PATH/runtimes"
            ;;
        add_llm)
    ...
    if [[ "$OPERATION_MODE" == "create_new" ]] || [[ "$OPERATION_MODE" == "add_llm" ]]; then
        ...
        safe_chown -R "$(id -u):$(id -g)" "$USB_BASE_PATH/.ollama/models"
