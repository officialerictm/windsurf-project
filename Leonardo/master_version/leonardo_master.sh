...
init_terminal() {
    ...
    if [ -z "$TERM" ] || [ "$TERM" = "dumb" ]; then
        export TERM=linux  # Fallback terminal type with basic color support
    fi
    
    # Set terminal to UTF-8 mode if possible
    export LANG=en_US.UTF-8
    export LC_ALL=en_US.UTF-8
    
    # Define color codes directly to avoid dependency on tput
    export ESC_START=$'\e['
    export ESC_END='m'
    export ESC_RESET="${ESC_START}0${ESC_END}"
    
    # Function to create color codes dynamically
    make_color() {
        printf "%s%s%s" "$ESC_START" "$1" "$ESC_END"
    }
    
    # Simple colors - these will always work even when UI file not loaded
    export RED=$(make_color "31")
    export GREEN=$(make_color "32")
    export YELLOW=$(make_color "33")
    export BLUE=$(make_color "34")
    export PURPLE=$(make_color "35")
    export CYAN=$(make_color "36")
    export WHITE=$(make_color "37")
    export BOLD=$(make_color "1")
    export RESET=$ESC_RESET
    
    # Test if colors are working
    TEST_COLOR="${GREEN}TEST${RESET}"
    DISPLAY_TEST=$(printf "%s" "$TEST_COLOR")
    
    # Basic direct escape sequence test
    printf "\e[32mColor test\e[0m\n" > /dev/null
    ...
    if [ -t 1 ]; then
        if [ -n "$(tput colors 2>/dev/null)" ]; then
            ...
            # Terminal doesn't appear to support colors, try simpler approach
            echo "Note: Limited color support detected. Some UI elements may not display correctly."
            USE_COLORS=1  # Still try to use colors
            export COLORTERM=0
        fi
    else
...
# Prevent direct sourcing of this file
if [ "$1" = "--no-run" ]; then
    return 0
fi
...
UI_FILE="${SCRIPT_DIR}/leonardo_ui.sh"
SIMPLE_UI_FILE="${SCRIPT_DIR}/leonardo_simple_ui.sh"

# Check terminal capability
check_terminal_capability() {
    # Test if ANSI colors work properly
    if [ -z "$TERM" ] || [ "$TERM" = "dumb" ]; then
    ...
    # Check if sudo is being used without preserving environment
    if [ -n "$SUDO_USER" ] && [ -z "$SUDO_PRESERVE_ENV" ]; then
        return 1
    fi
...
# Choose the appropriate UI file
if check_terminal_capability && [ -f "${UI_FILE}" ] && [ -r "${UI_FILE}" ]; then
    # Terminal supports advanced features, use the fancy UI
    ACTIVE_UI_FILE="${UI_FILE}"
    echo "Using standard UI with color support"
    USE_FANCY_UI=1
else
    # Terminal has limited capabilities, use the simple UI
    if [ -f "${SIMPLE_UI_FILE}" ] && [ -r "${SIMPLE_UI_FILE}" ]; then
        ACTIVE_UI_FILE="${SIMPLE_UI_FILE}"
        echo "Using simple UI for better compatibility"
        USE_FANCY_UI=0
    else
        # Fallback to standard UI if simple UI isn't available
        ACTIVE_UI_FILE="${UI_FILE}"
        echo "Warning: Using standard UI, but terminal may not display correctly"
        USE_FANCY_UI=1
    fi
fi

# Source the selected UI file
if [ -z "${LEONARDO_UI_LOADED}" ] && [ -z "${LEONARDO_SIMPLE_UI_LOADED}" ]; then
    if ! source "${ACTIVE_UI_FILE}"; then
        echo "Error: Failed to source UI components from ${ACTIVE_UI_FILE}" >&2
        exit 1
    fi
    
    # Export all functions for use in subshells
    if command -v export_functions >/dev/null 2>&1; then
        export_functions
    fi
    
    # Verify essential functions are available
    if ! command -v show_header >/dev/null 2>&1 || ! command -v llama_speak >/dev/null 2>&1; then
        echo "Error: Failed to load required functions from ${ACTIVE_UI_FILE}" >&2
        exit 1
    fi
fi
...
show_llama_message() {
    local message="$1"
    local severity="${2:-1}"  # Default to normal severity (1)
    local mood="normal"
    ...
    # Call the centralized llama_speak function from leonardo_ui.sh
    if command -v llama_speak >/dev/null 2>&1; then
        llama_speak "$mood" "$message"
    else
        # Fallback if llama_speak is somehow not available
        echo "Llama: $message" # Basic fallback
    fi
}
...
# List available AI models and handle user selection
list_models() {
    while true; do
        clear_screen
        ...
        # Validate input
        if [[ ! "$choice" =~ ^[1-5]$ ]]; then
            llama_speak "warning" "Invalid selection. Please choose a number between 1-5 or 'b' to go back."
            sleep 1
            continue
        fi
        ...
        echo -e "\n${C_WHITE_BOLD}Would you like to select another model? (y/N): ${C_RESET}"
        read -r another
