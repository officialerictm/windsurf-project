...
# Generated on: 2025-05-31 16:46:28
...
show_main_menu() {
    ...
    while true; do
        ...
        echo -e "  ${CYAN}1.${NC} Create New Leonardo AI USB"
        echo -e "  ${CYAN}2.${NC} Add Models to Existing USB"
        echo -e "  ${CYAN}3.${NC} Check USB Health"
        echo -e "  ${CYAN}4.${NC} List Available Models"
...
create_new_usb() {
    ...
    if [[ $EUID -ne 0 ]]; then
        ...
        log_message "ERROR" "USB creation failed: not running as root"
        return 1
    fi
    
    # Show initial warning with friendly llama
    show_warning "This process will create a new Leonardo AI USB device." "normal"
    echo -e "${YELLOW}(•ᴗ•)🦙${NC} ${YELLOW}You will need to select a USB device to format.${NC}"
    echo -e "     Make sure you have backed up any important data."
    echo ""
    
    # Confirm initial action
    if ! confirm_action "Continue with USB creation?"; then
        echo -e "\n${YELLOW}Operation cancelled.${NC}"
        log_message "INFO" "USB creation cancelled by user at initial confirmation"
        return 0
    fi
    ...
    if [[ -z "$device" ]]; then
        ...
        log_message "INFO" "USB creation cancelled: no device selected"
        return 0
    fi
    ...
    if ! verify_usb_device "$device"; then
        ...
        log_message "INFO" "USB creation cancelled: device verification failed"
        return 0
    fi
...
check_system_requirements() {
    ...
    if [[ $EUID -ne 0 ]]; then
        log_message "WARNING" "Not running as root, some features may be limited"
        echo -e "${YELLOW}Warning: Not running as root.${NC}"
        echo -e "Some features like USB formatting may not work properly."
        echo -e "Consider running this script with sudo for full functionality."
        echo ""
        
        # Prompt to continue
        if ! confirm_action "Continue anyway?"; then
            log_message "INFO" "User chose to exit due to insufficient privileges"
            exit 0
        fi
    fi
