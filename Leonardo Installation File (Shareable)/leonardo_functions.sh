...
fancy_download() {
    ...
    # First, check if this is a known model or binary based on description (highest priority)
    if [[ "$description" == *"phi3:mini"* ]]; then
        total_size=2470576128  # 2.3GB for phi3:mini
        size_str="~2.3GB (from pre-flight check)"
        print_info "Using pre-flight size for phi3:mini model: $size_str"
    elif [[ "$description" == *"llama3:8b"* ]]; then
        total_size=5042946560  # 4.7GB for llama3:8b
        size_str="~4.7GB (from pre-flight check)"
        print_info "Using pre-flight size for llama3:8b model: $size_str"
    elif [[ "$description" == *"codellama:7b"* ]]; then
        total_size=4080374784  # 3.8GB for codellama:7b
        size_str="~3.8GB (from pre-flight check)"
        print_info "Using pre-flight size for codellama:7b model: $size_str"
    # Check for Ollama binaries
    elif [[ "$description" == *"Linux Ollama"* ]]; then
        total_size=1685774336  # ~1.57GB for Linux Ollama
        size_str="~1.57GB (estimated)"
        print_info "Using pre-flight size for Linux Ollama: $size_str"
    elif [[ "$description" == *"macOS Ollama"* ]]; then
        total_size=110250000  # ~110MB for macOS Ollama
        size_str="~110MB (estimated)"
        print_info "Using pre-flight size for macOS Ollama: $size_str"
    elif [[ "$description" == *"Windows Ollama"* ]]; then
        total_size=138529000  # ~139MB for Windows Ollama
        size_str="~139MB (estimated)"
        print_info "Using pre-flight size for Windows Ollama: $size_str"
    # Otherwise try other size detection methods
    elif command -v curl &> /dev/null; then
        # Try to get size from HTTP header
        ...
        if [[ -n "$content_length" && "$content_length" -gt 0 ]]; then
            ...
            size_lookup["ollama-linux-amd64"]="1685774336"     # ~1.57GB
            ...
            if [[ -z "$size_str" || "$size_str" == "0" ]]; then
                # Skip model check (already done at beginning of function)
...
format_usb_exfat() {
    ...
    # Enhanced USB emergency release procedure - brute force approach
    print_info "Performing emergency USB release procedure for $device_path..."
    
    # Step 1: Aggressive unmounting of any mounted partitions
    print_info "1/7: Unmounting all partitions with increasing force levels..."
    
    # Level 1: Standard unmount of all partitions from the device
    ...
    # Level 2: Force unmount with lazy option
    print_info "   Applying lazy unmount for any remaining mounts..."
    for part in $(lsblk -lno NAME | grep -E "^$(basename "$device_path")[0-9p]*$"); do
        sudo umount -l "/dev/$part" 2>/dev/null || true
    done
    
    # Level 3: Force unmount any remaining mounts by device path
    ...
    # Level 4: Direct force unmount of main device
    sudo umount -f "$device_path"* 2>/dev/null || true

    # Step 2: Aggressive process termination
    print_info "2/7: Terminating ALL processes using the device..."
    local device_parts=$(lsblk -ln -o NAME "$device_path" 2>/dev/null | grep -v "^$(basename "$device_path")$" || true)

    # Kill processes using the main device with extreme prejudice
    if command -v fuser >/dev/null 2>&1; then
        print_info "   Using fuser to kill all processes using the device..."
        sudo fuser -k "$device_path"* 2>/dev/null || true
    fi

    # More targeted killing with lsof
    local device_in_use=$(lsof "$device_path" 2>/dev/null || true)
    if [ -n "$device_in_use" ]; then
        print_warning "   Device $device_path is in use by processes, forcefully terminating..."
    ...
    # Step 3: Force system caches to flush
    print_info "3/7: Flushing all system caches and buffers..."
    sync
    print_info "   Forcing kernel to drop all caches..."
    
    # Try multiple buffer flushing methods
    if command -v blockdev >/dev/null 2>&1; then
        sudo blockdev --flushbufs "$device_path" 2>/dev/null || true
    fi
    
    # Step 4: Additional device reset procedures
    print_info "4/7: Applying hardware-level device reset procedures..."
    
    # Try hdparm if available for low-level disk reset
    if command -v hdparm >/dev/null 2>&1; then
        print_info "   Using hdparm to force hardware reset..."
        sudo hdparm -z "$device_path" 2>/dev/null || true
    fi
    
    # Try eject if available
    if command -v eject >/dev/null 2>&1; then
        print_info "   Using eject to force device reset..."
        sudo eject -s "$device_path" 2>/dev/null || true
        sleep 1
    fi

    # Step 5: Last resort - direct drive wiping
    print_info "5/7: Performing direct drive wiping of partition signatures..."
    
    # Use dd to wipe the first 10MB of the drive - this often clears stubborn partition tables
    print_info "   Using direct low-level write to clear partition tables..."
    sudo dd if=/dev/zero of="$device_path" bs=1M count=10 2>/dev/null || true
    sync
    sleep 2
    
    # Now use wipefs for a more targeted approach
    print_info "6/7: Wiping all filesystem signatures..."
    sudo wipefs -a -f "$device_path" 2>/dev/null || true
    
    # Step 7: Final settling period
    print_info "7/7: Allowing device to settle..."
    sync
    sleep 3
    
    # Request kernel to rescan the device
    if command -v udevadm >/dev/null 2>&1; then
        sudo udevadm settle
        sudo udevadm trigger --subsystem-match=block
    fi
    
    print_success "Emergency USB release procedure complete - device should now be free"
    sleep 1

    # Create new partition table and partition with fallback mechanism
    # Using MBR/MSDOS partition table for better Windows compatibility
    print_info "Crafting new MS-DOS partition table for Windows compatibility..."
    local partition_table_result=$(sudo parted -s "$device_path" mklabel msdos 2>&1 || true)
    ...
    if $partition_error; then
        ...
        if command -v sfdisk >/dev/null 2>&1; then
            ...
            echo 'label: dos' | sudo sfdisk --force "$device_path" 2>/dev/null || true
            sleep 1
        fi
    fi

    # Always try to create the partition, even if there were issues with the partition table
    print_info "Crafting primary partition with FAT32/exFAT compatible type..."
    
    # Use specific partition type for better Windows compatibility
    # Set the partition type to 0x0c (FAT32 LBA) which Windows recognizes well for USB drives
    local create_partition_result=$(sudo parted -s -a optimal "$device_path" mkpart primary 1MiB 100% 2>&1 || true)
    
    # Try to set the partition type to a Windows-compatible one (0x0C = FAT32 LBA or 0x07 = exFAT)
    if command -v fdisk >/dev/null 2>&1; then
        print_info "Setting Windows-compatible partition type..."
        echo -e "t\n0c\nw" | sudo fdisk "$device_path" 2>/dev/null || true
        sync; sleep 1
    fi
    ...
    # Try different formatting tools in order of preference with Windows-compatible parameters
    if command -v mkfs.exfat &>/dev/null; then
        print_info "Using mkfs.exfat with Windows-compatible parameters..."
        # Use specific sector size and cluster size settings for best Windows compatibility
        # -s 512 = standard sector size for Windows
        # -b 32768 = 32KB cluster size (good balance for exFAT performance and compatibility)
        if sudo mkfs.exfat -s 512 -b 32768 -n "$label" "$partition_path" 2>/dev/null; then
            format_success=true
            # Additional verification for Windows compatibility
            print_info "Verifying exFAT formatting for Windows compatibility..."
            sleep 2
            sync
        else
    ...
    if ! $format_success && command -v mkfs.vfat &>/dev/null; then
        ...
        print_info "Using mkfs.vfat (FAT32) with Windows-optimized parameters..."
        
        # Use FAT32 with optimal Windows parameters
        # -F 32 = FAT32
        # -S 512 = 512 bytes per sector (Windows standard)
        # -a = Align correctly for best Windows compatibility
        if sudo mkfs.vfat -F 32 -S 512 -a -n "$label" "$partition_path" 2>/dev/null; then
            print_warning "USB formatted as FAT32 instead of exFAT. Some large files may not be supported."
            print_info "FAT32 formatting successful with Windows-compatible parameters."
            format_success=true
...
# Helper function to offer Windows-compatible formatting and handle the user's choice
offer_windows_compat_formatting() {
    local device_path="$1"
    local label="${2:-LEONARDO}"
    local format_choice_var="$3"
    
    # Get the current value of FORMAT_USB_CHOICE from the caller
    local current_format_choice=$(eval echo "\$$format_choice_var")
    
    if [[ "$current_format_choice" != "yes" ]]; then
        # No formatting requested, just return
        return 0
    fi
    
    # Ask if they want to use Windows-compatible formatting
    local WINDOWS_COMPAT_CHOICE=""
    print_header "🪟 WINDOWS COMPATIBILITY OPTIONS 🪟"
    print_info "The standard formatting method sometimes causes Windows compatibility issues."
    print_info "If you plan to use this USB with Windows systems, you may want to use our"
    print_info "emergency Windows-compatible formatting method, which provides better"
    print_info "compatibility with Windows machines."
    
    ask_yes_no_quit "Would you like to use Windows-compatible formatting?" WINDOWS_COMPAT_CHOICE "no"
    
    if [[ "$WINDOWS_COMPAT_CHOICE" == "yes" ]]; then
        print_info "Using Windows-compatible formatting for $device_path..."
        if emergency_format_usb_for_windows "$device_path" "$label"; then
            print_success "Windows-compatible formatting completed successfully."
            eval "$format_choice_var=\"completed\""  # Skip regular formatting
            return 0
        else
            print_error "Windows-compatible formatting failed."
            print_info "Would you like to try standard formatting instead?"
            
            local CONTINUE_WITH_STANDARD=""
            ask_yes_no_quit "Continue with standard formatting?" CONTINUE_WITH_STANDARD "yes"
            
            if [[ "$CONTINUE_WITH_STANDARD" != "yes" ]]; then
                print_info "Formatting cancelled by user."
                eval "$format_choice_var=\"no\"" 
                return 1
            fi
        fi
    fi
    
    # If we get here, continue with standard formatting
    return 0
}
...
# Function for emergency Windows-compatible USB formatting
# This function provides a more aggressive approach when standard formatting fails
# It ensures Windows compatibility by using MS-DOS partition tables and Windows-friendly parameters
emergency_format_usb_for_windows() {
    # Important: Save current error handling settings and temporarily disable error exit
    local old_errexit_setting=$(set +o | grep errexit)
    set +e  # Disable exit on error for this function
    ...
    print_header "🛠️ EMERGENCY WINDOWS-COMPATIBLE USB FORMATTER 🛠️"
    print_info "Starting aggressive USB formatting procedure for Windows compatibility"
    print_info "This procedure uses extreme measures to ensure the drive works on Windows"

    # Step 1: Ultra-aggressive device unmounting
    print_info "Step 1/7: Ultra-aggressive device unmounting..."
    
    # Force unmount every possible mount point
    for attempt in {1..3}; do
        sudo umount -f "$device_path"* 2>/dev/null || true
        sleep 1
    done

    # Step 2: Absolutely terminate any process using the device
    print_info "Step 2/7: Force-killing ALL processes using the device..."
    sudo fuser -km "$device_path"* 2>/dev/null || true
    sleep 2

    # Step 3: Brute force wiping of drive beginning
    print_info "Step 3/7: Direct low-level drive wiping (first 20MB)..."
    sudo dd if=/dev/zero of="$device_path" bs=1M count=20 status=none 2>/dev/null || true
    sync
    sleep 2

    # Step 4: Create MS-DOS partition table (Windows-compatible)
    print_info "Step 4/7: Creating Windows-compatible MS-DOS partition table..."
    sudo parted -s "$device_path" mklabel msdos 2>/dev/null || true
    sleep 2

    # Step 5: Create properly aligned primary partition
    print_info "Step 5/7: Creating properly aligned primary partition..."
    sudo parted -s -a optimal "$device_path" mkpart primary 1MiB 100% 2>/dev/null || true
    sleep 2

    # Force Windows-compatible partition type
    echo -e "t\n0c\nw" | sudo fdisk "$device_path" 2>/dev/null || true

    # Step 6: Force kernel to recognize new partition table
    print_info "Step 6/7: Forcing kernel to recognize new partition table..."
    sudo partprobe "$device_path" 2>/dev/null || true
    ...
    if [ ! -b "$partition_path" ]; then
        print_error "Failed to create partition $partition_path after multiple attempts"
        eval "$old_errexit_setting"  # Restore original error handling
        return 1
    fi

    # Step 7: Format with exFAT using Windows-compatible parameters
    print_info "Step 7/7: Formatting with Windows-compatible exFAT parameters..."
    if command -v mkfs.exfat &>/dev/null; then
        echo "--- DEBUG: TOP OF MKFS.EXFAT BLOCK ---" >&2
        exit 1
        print_info "DEBUG: mkfs.exfat command IS available." # Added for clarity
        # Use specific sector and cluster sizes for Windows compatibility
        print_info "DEBUG: Attempting mkfs.exfat on $partition_path with label $label" # Added
        echo "DEBUG: Full command: sudo mkfs.exfat -s 512 -b 32768 -n \"$label\" \"$partition_path\"" # Added
        
        # Capture stdout and stderr to temporary files
        sudo mkfs.exfat -s 512 -b 32768 -n "$label" "$partition_path" > /tmp/mkfs_stdout.log 2> /tmp/mkfs_stderr.log
        local mkfs_exit_code=$?
        echo "--- DEBUG: BOTTOM OF MKFS.EXFAT BLOCK ---" >&2
        exit 1
        print_info "DEBUG: mkfs.exfat STDOUT:" # Added
        cat /tmp/mkfs_stdout.log # Added
        print_info "DEBUG: mkfs.exfat STDERR:" # Added
        cat /tmp/mkfs_stderr.log # Added
        print_info "DEBUG: mkfs.exfat exit code: $mkfs_exit_code" # Added

        if [ $mkfs_exit_code -eq 0 ]; then
            formatting_success=true
        else
            # Try FAT32 as a last resort
            print_warning "exFAT formatting failed (exit code $mkfs_exit_code), attempting FAT32 fallback..." # Modified
            if command -v mkfs.vfat &>/dev/null; then
                sudo mkfs.vfat -F 32 -S 512 -n "$label" "$partition_path" 2>/dev/null
                if [ $? -eq 0 ]; then
                    formatting_success=true
                    print_warning "USB formatted as FAT32 instead of exFAT. Some large files may not be supported."
                fi
        ...
        # If mkfs.exfat is not available, try vfat
        print_warning "mkfs.exfat not found, attempting FAT32 formatting..."
        if command -v mkfs.vfat &>/dev/null; then
            sudo mkfs.vfat -F 32 -S 512 -n "$label" "$partition_path" 2>/dev/null
            if [ $? -eq 0 ]; then
                formatting_success=true
    ...
    if $formatting_success; then
        print_success "Emergency Windows-compatible formatting completed successfully!"
        print_info "The USB drive should now be recognized properly on Windows systems."
        eval "$old_errexit_setting"  # Restore original error handling
        return 0
    else
        print_error "Emergency formatting failed for $partition_path"
        print_info "You may need to try again after rebooting your system."
        eval "$old_errexit_setting"  # Restore original error handling
        return 1
    fi
}

# Helper function to prompt for Windows-compatible formatting
prompt_for_windows_formatting() {
    local device_path="$1"
    local label="$2"
    local format_choice_var="$3"
...
emergency_format_usb_for_windows() {
    ...
    if command -v mkfs.exfat &>/dev/null; then
        # Use specific sector and cluster sizes for Windows compatibility
        sudo mkfs.exfat -s 512 -b 32768 -n "$label" "$partition_path" 2>/dev/null
        if [ $? -eq 0 ]; then
            formatting_success=true
        else
            # Try FAT32 as a last resort
            print_warning "exFAT formatting failed, attempting FAT32 fallback..."
            if command -v mkfs.vfat &>/dev/null; then
                sudo mkfs.vfat -F 32 -S 512 -n "$label" "$partition_path" 2>/dev/null
                if [ $? -eq 0 ]; then
                    formatting_success=true
...
ask_format_usb() {
    ...
    if [[ "$FORMAT_USB_CHOICE" == "yes" ]]; then
        # Offer Windows-compatible formatting
        prompt_for_windows_formatting "$usb_device" "$usb_label" "FORMAT_USB_CHOICE"
    fi
...
calculate_estimated_binary_size_bytes() {
    ...
    local -A base_sizes=(
        [linux]=$((1607 * 1024 * 1024))     # 1.57GB for Linux
        ...
        if ! command -v "$cmd" &> /dev/null; then
            if [[ "$cmd" == "mkfs.exfat" ]] && (command -v mkexfatfs &> /dev/null); then
                ...
                continue
                        ...
                        # BEGIN: Added logic for mkfs.exfat auto-installation
            if [[ "$cmd" == "mkfs.exfat" && "$os_type" == "Linux" && -n "$pkg_manager_name" ]]; then
                local pkg_to_install_exfat="exfatprogs" # Common package name for mkfs.exfat

                # Check if print_prompt and other UI functions exist, otherwise use echo
                local prompt_func="echo -e"
                local info_func="echo"
                local success_func="echo"
                local error_func="echo"
                if command -v print_prompt &> /dev/null; then prompt_func="print_prompt"; fi
                if command -v print_info &> /dev/null; then info_func="print_info"; fi
                if command -v print_success &> /dev/null; then success_func="print_success"; fi
                if command -v print_error &> /dev/null; then error_func="print_error"; fi

                echo # Newline for readability before prompt
                local install_exfat_choice
                if [[ "$prompt_func" == "print_prompt" ]]; then
                    "$prompt_func" "Command '${C_BOLD}mkfs.exfat${C_RESET}' (for exFAT formatting) is missing.\n  Package '${C_GREEN}${pkg_to_install_exfat}${C_RESET}' can provide it.\n  Attempt to install using ${C_BLUE}${pkg_manager_name}${C_RESET}? (y/n): "
                else
                    # Manual prompt construction if print_prompt is not available
                    echo -e "  ${C_YELLOW}⚠️ WARNING:${C_RESET} Command '${C_BOLD}mkfs.exfat${C_RESET}' (for exFAT formatting) is missing."
                    echo -e "  Package '${C_GREEN}${pkg_to_install_exfat}${C_RESET}' can provide it."
                    echo -e "  Attempt to install using ${C_BLUE}${pkg_manager_name}${C_RESET}? (y/n): \c"
                fi
                read -r install_exfat_choice
                echo # Newline after read

                if [[ "$install_exfat_choice" =~ ^[Yy]$ ]]; then
                    "$info_func" "  Attempting to install '$pkg_to_install_exfat' using $pkg_manager_name..."
                    
                    if [[ "$pkg_manager_name" == "apt" ]] || [[ "$pkg_manager_name" == "apt-get" ]]; then
                        "$info_func" "    Running package list update (e.g., sudo apt-get update)..."
                        if sudo apt-get update -qq; then
                            "$success_func" "    Package list update successful."
                        else
                            "$error_func" "    Package list update failed. Installation of $pkg_to_install_exfat might also fail."
                        fi
                    fi
                    
                    # pkg_manager_cmd should already include sudo and relevant flags like -y
                    local full_install_cmd="$pkg_manager_cmd $pkg_to_install_exfat"
                    "$info_func" "    Executing: ${C_DIM}${full_install_cmd}${C_RESET}"

                    if eval "$full_install_cmd"; then
                        "$success_func" "  Installation of '$pkg_to_install_exfat' reported as successful."
                        if command -v "$cmd" &> /dev/null; then # Re-check for mkfs.exfat
                            echo -e "  $dep_found_msg '$cmd' is now available."
                            continue # Successfully installed and found, skip to next dependency in the main loop
                        else
                            "$error_func" "  $dep_fail_msg '$cmd' is STILL NOT available after '$pkg_to_install_exfat' installation attempt."
                            # Falls through to be added to missing_deps_cmds / missing_deps_pkgs
                        fi
                    else
                        "$error_func" "  $dep_fail_msg Installation of '$pkg_to_install_exfat' failed."
                        # Falls through
                    fi
                else
                    "$info_func" "  Skipping automatic installation of '$pkg_to_install_exfat'."
                    # Falls through
                fi
            fi
            # END: Added logic for mkfs.exfat auto-installation
...
# Function to generate the proxy server script that helps avoid CORS issues
generate_proxy_server() {
    local usb_base_dir="$1"
    local proxy_server_file="$usb_base_dir/webui_server.js"

    print_info "Generating proxy server (webui_server.js) to avoid CORS issues..."

    cat > "$proxy_server_file" << 'PROXY_SERVER_CONTENT'
#!/usr/bin/env node
// Ultra-simple HTTP server that serves the WebUI and proxies API requests
// With minimal dependencies and explicit file serving

const http = require('http');
const fs = require('fs');
const path = require('path');

// Configuration - using a different port to avoid conflicts
const PORT = 8181;
...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith('/api/')) {
    console.log(`Proxying API request to Ollama: ${req.url}`);
    ...
    proxyReq.on('error', (error) => {
      ...
      res.writeHead(500);
      res.end(JSON.stringify({ error: `Failed to connect to Ollama API: ${error.message}` }));
    });
  ...
  // Handle root request - serve the index.html file
  if (req.url === '/' || req.url === '') {
  ...
  // For any other request, check if it exists in the webui directory
  const filePath = path.join(WEBUI_DIR, req.url.replace(/^\//, ''));
...
// Start the server
server.listen(PORT, () => {
  console.log(`Leonardo AI USB Proxy Server running at http://localhost:${PORT}/`);
  console.log(`Proxying API requests to http://${OLLAMA_HOST}:${OLLAMA_PORT}/`);
  console.log(`WebUI directory: ${WEBUI_DIR}`);
});
PROXY_SERVER_CONTENT

    # Make the proxy server script executable
    chmod +x "$proxy_server_file"

    print_success "Proxy server script created successfully at $proxy_server_file"
}
...
generate_webui_html() {
    ...
    print_info "Generating Web UI (index.html) with proxy server support..."
...
<body>
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            try {
                console.log('Sending chat request to proxy server');
...
if check_dependency "node"; then
    ...
    # Make sure the proxy server script exists
    if [ ! -f "\$SCRIPT_DIR/webui_server.js" ]; then
        printf "%b\\n" "\${C_YELLOW}⚠️ Proxy server script not found at \$SCRIPT_DIR/webui_server.js\${C_RESET}";
        printf "%b\\n" "\${C_YELLOW}Will use direct file access instead (may have CORS issues).\${C_RESET}";
    else
        ...
        if curl -s "http://localhost:\$WEBUI_PROXY_PORT/" >/dev/null 2>&1; then
            printf "%b\\n" "\${C_GREEN}✅ WebUI proxy server started with PID: \$PROXY_PID\${C_RESET}";
            USE_PROXY=true
        else
            printf "%b\\n" "\${C_RED}❌ Proxy server failed to start properly.\${C_RESET}";
            printf "%b\\n" "\${C_YELLOW}Will use direct file access instead (may have CORS issues).\${C_RESET}";
        fi
    fi
fi

# Set the WebUI path based on whether we're using the proxy or not
if [ "\$USE_PROXY" = true ]; then
    WEBUI_PATH="http://localhost:\$WEBUI_PROXY_PORT/";
else
...
generate_usb_files() {
    ...
    mkdir -p "$usb_base_dir/webui" "$usb_base_dir/runtimes"
    generate_webui_html "$usb_base_dir" "$default_model_hint"
    generate_proxy_server "$usb_base_dir"  # Generate our CORS-avoiding proxy server
