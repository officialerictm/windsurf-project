...
while true; do
    ...
    if [[ "$OPERATION_MODE" == "create_new" ]]; then
        ...
        # Offer Windows-compatible formatting if the user chose to format
        offer_windows_compat_formatting "$USB_DEVICE_PATH" "$USB_LABEL" "FORMAT_USB_CHOICE"
    ...
    case "$OPERATION_MODE" in
        ...
        create_new)
            ...
            if [[ "$FORMAT_USB_CHOICE" == "yes" ]]; then
                ...
                if [[ "$WINDOWS_COMPAT_CHOICE" == "yes" ]]; then
                    print_info "Using Windows-compatible formatting for $USB_DEVICE_PATH..."
                    if emergency_format_usb_for_windows "$USB_DEVICE_PATH" "$USB_LABEL"; then
                        print_success "Windows-compatible formatting completed successfully."
                        FORMAT_USB_CHOICE="completed"  # Skip regular formatting
                    else
                        print_error "Windows-compatible formatting failed."
                        print_info "Would you like to try standard formatting instead?"
                ...
                # If we're still set to format (not completed by Windows method and not cancelled)
                if [[ "$FORMAT_USB_CHOICE" == "yes" ]]; then
